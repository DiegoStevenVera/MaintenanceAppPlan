...
- Roles: Maintenance Engineer, Coordinator, Boss, Administrator. "Supervisor" in older drafts means Boss only when it refers to read-only leadership; operational closure belongs to Coordinator.
...
| **Frustrations** | Incomplete reports, missing signatures, maintenance engineers skipping fields, not knowing what happened during the shift. |
...
| **Primary goal** | Track tool check-out/check-in, manage spare part stock, process replacements reported by maintenance engineers. |
| **Frustrations** | Discrepancies between reported replacements and actual stock, maintenance engineers not reporting parts used, unclear replacement history. |
...
| **Permissions** | Maintenance Engineer capabilities plus close/reopen maintenance activities and create/edit/confirm PCON schedules. Future: import PCON updates and manage documentation workflows. |
...
| **Permissions** | All capabilities from Maintenance Engineer, Coordinator, and Boss, plus administrative configuration. |
...
| Basic preventive schedule view | Yes | Maintenance Engineers need to see assigned PMs |
| Create preventive report (draft) | Yes | Replace paper PM checklists |
| Submit preventive report | Yes | Finalize PM documentation |
| User roles (Maintenance Engineer/Coordinator/Boss/Administrator) | Yes | Basic access control |
...
| Push notifications | Alert maintenance engineers of new assignments |
...
| Geofencing for asset location | Verify maintenance engineer is at correct location |
...
- Maintenance Engineers initially use shared iPads. iPhone support is future and depends on corporate security approval.
...
- The asset hierarchy is pre-loaded by the administrator before maintenance engineer use.
...
- The imported PCON plan supplies broad month assignments. Coordinator and
  Administrator can change an unconfirmed month, create exact weekly proposals,
  and atomically confirm the full weekly meeting block. Maintenance Engineer
  and Boss have read-only access.
...
| **Actor** | Maintenance Engineer, Coordinator, Boss, Administrator |
...
|-------|-------|
| **ID** | US-COR-01 |
| **Title** | Maintenance Engineer or Coordinator creates a corrective maintenance event |
| **Actor** | Maintenance Engineer, Coordinator |
...
| **Acceptance criteria** | Event is created. Maintenance Engineer/Coordinator can start it. Boss can view it read-only. |
| **Priority** | P0 |
...
| **Main flow** | 1. User taps "Corrective" tab. 2. App displays events by status. 3. Maintenance Engineer sees operational events. 4. Coordinator sees events for review/closure. 5. Boss sees read-only events and metrics. 6. User taps event to open detail. |
...
|-------|-------|
| **ID** | US-COR-04 |
| **Title** | Maintenance Engineer starts working on a corrective event |
| **Actor** | Maintenance Engineer, Coordinator |
...
|-------|-------|
| **ID** | US-COR-05 |
| **Title** | Maintenance Engineer creates or edits a corrective shift report version |
| **Actor** | Maintenance Engineer, Coordinator |
| **Preconditions** | Event is IN_PROGRESS or COMPLETED, not CLOSED. |
| **Main flow** | 1. User taps "Create/Edit Report". 2. System opens a dynamic corrective report based on the real format. 3. User fills event, failure, impact, activities, tests, attachments, and conclusions. 4. User adds activities performed. 5. If activity type is Component Replacement, replacement fields appear inside that activity block. 6. User selects removed asset, installed asset, source, destination, and reason. 7. User captures participant signatures on the shared iPad. 8. User optionally sets Stop Here marker. 9. User finalizes the current report version. 10. System saves the immutable snapshot and updates the timeline. 11. From the report-version detail, an authorized user generates/downloads the PDF. 12. From the maintenance detail screen, authorized users can read and add comments scoped only to this corrective event. |
...
| **Acceptance criteria** | Report uses dynamic blocks. Replacement fields appear only for replacement activities. Finalizing creates a new version; the version screen can generate/download its PDF. Previous versions remain visible. |
| **Priority** | P0 |
...
|-------|-------|
| **ID** | US-COR-06 |
| **Title** | Maintenance Engineer completes and Coordinator closes a corrective event |
| **Actor** | Maintenance Engineer (complete), Coordinator (close/reopen) |
| **Preconditions** | Event is IN_PROGRESS. At least one report version exists. |
| **Main flow** | 1. Maintenance Engineer taps "Complete/Resolve". 2. Adds notes if needed. 3. System transitions event to COMPLETED. 4. Coordinator reviews event. 5. Coordinator taps "Close Event". 6. System transitions event to CLOSED. |
...
|-------|-------|
| **ID** | US-COR-07 |
| **Title** | Maintenance Engineer marks Stop Here for report/PDF filtering |
...
|-------|-------|
| **ID** | US-COR-08 |
| **Title** | Maintenance Engineer continues corrective documentation in the next shift |
| **Actor** | Maintenance Engineer |
| **Preconditions** | Corrective event is not CLOSED. Previous shift may contain Stop Here marker. |
| **Main flow** | 1. Maintenance Engineer opens event detail. 2. Reviews previous report versions and marker. 3. Creates/edits the report for current shift. 4. Continues documentation and may correct previous fields if needed. 5. Finalizes a new report version. |
...
|-------|-------|
| **ID** | US-COMP-02 |
| **Title** | Maintenance Engineer installs a component in an equipment slot (via ReplacementTask) |
| **Actor** | Maintenance Engineer |
...
| **Main flow** | 1. During corrective report creation, maintenance engineer adds a ReplacementTask. 2. Maintenance Engineer taps "Select Component". 3. System shows available EN_STOCK components matching slot requirements (filtered by ComponentType). 4. Maintenance Engineer selects component. 5. System auto-assigns to current slot (based on task context). 6. Component status changes to INSTALLED. 7. Slot shows as occupied. 8. ComponentMovement recorded (INSTALL). |
...
|-------|-------|
| **ID** | US-COMP-03 |
| **Title** | Maintenance Engineer removes a component from its slot (via ReplacementTask) |
| **Actor** | Maintenance Engineer |
| **Preconditions** | Corrective event is IN_PROGRESS. ReplacementTask is being performed. Component is INSTALLED in the slot. |
| **Main flow** | 1. During corrective report creation, maintenance engineer selects "Remove Component". 2. System shows currently installed component in the slot. 3. Maintenance Engineer confirms removal. 4. Component status changes to REMOVED (or EN_REPARACIÃ“N if sent for repair). 5. Slot becomes empty. 6. ComponentMovement recorded (REMOVE). 7. Asset lifecycle updated (AssetReplacement record created). |
...
|-------|-------|
| **ID** | US-PREV-01 |
| **Title** | Maintenance Engineer views assigned preventive maintenance schedule |
| **Actor** | Maintenance Engineer |
| **Preconditions** | Preventive schedule exists. Maintenance Engineer is assigned tasks. |
| **Main flow** | 1. Maintenance Engineer taps "Preventive" tab. 2. App displays list of scheduled activities. 3. Each row shows: title, due date, asset, priority, status badge. 4. List grouped by: Overdue (red), Today, This Week, This Month. 5. User taps an activity â†’ navigates to schedule detail with template steps. |
...
|-------|-------|
| **ID** | US-REPLACE-01 |
| **Title** | Maintenance Engineer records that a component was replaced during maintenance |
| **Actor** | Maintenance Engineer |
...
### US-ASSIGN-01: Assign Maintenance Engineer to Event

| Field | Value |
|-------|-------|
| **ID** | US-ASSIGN-01 |
| **Title** | Coordinator assigns one or more maintenance engineers to a corrective event |
| **Actor** | Coordinator |
| **Preconditions** | Event exists in OPEN status. Maintenance Engineers exist in the system. |
| **Main flow** | 1. Coordinator opens event detail. 2. Taps "Assign Maintenance Engineers". 3. Shows list of available maintenance engineers. 4. Coordinator selects one or more. 5. Taps "Confirm". 6. Maintenance Engineers receive event in their corrective list. |
| **Alternate flows** | **Reassign:** Coordinator can remove and reassign maintenance engineers. |
| **Acceptance criteria** | Maintenance Engineer list loads. Multi-select works. Maintenance Engineers see event immediately after assignment. |
| **Priority** | P1 |
...
[Corrective Tab â†’ Event List]
    ...
    â”‚
    â””â”€â”€ [Tap "+" (Maintenance Engineer/Coordinator)] â”€â”€â†’ [Create Event Form]
                                        â”‚
                                        ...
                                        â””â”€â”€ [Tap "Create"] â”€â”€â†’ [Event created (OPEN)]
                                              â”‚
                                              â””â”€â”€ [Assign maintenance engineers] â”€â”€â†’ [Select from list]
```
...
| Screen | Purpose | Primary Actions | Data Displayed | Entry Points | API Endpoints | Required Role | Offline Behavior | Media |
|--------|---------|----------------|----------------|--------------|---------------|---------------|------------------|-------|
| **Event List** | View open events | Filter, pull-to-refresh, create event (+ for Maintenance Engineer/Coordinator/Administrator) | Event rows with severity, status, asset, time | Corrective tab | `GET /corrective-events` | Engineer/Coordinator/Administrator/Boss read-only | Show cached list | None |
| **Event Detail** | View event info & timeline | Start, resolve, close, create report, assign engineers | Status, severity, description, asset, timeline, reports | Event list tap | `GET /corrective-events/{id}` | Engineer/Coordinator/Boss | Show cached timeline | Initial photo |
| **Create Event** | Start new corrective event | Select subsystem, business-anchor equipment and affected descendant asset; set severity and SAP data | Searchable equipment list, normalized asset tree, read-only project/location context | Event list â†’ + | `GET /corrective-events/creation-context`, `POST /corrective-events` | Maintenance Engineer/Coordinator/Administrator | Save as draft locally | Initial photo |
| **Assign Maintenance Engineers** | Assign engineers to event | Select/deselect engineers, confirm | Available engineers list | Event Detail â†’ Assign | `POST /corrective-events/{id}/assign` planned | Coordinator | Not available offline | None |
| **Report Form** | Fill shift report | Add tasks, photos, signatures, replacements, submit | Tasks, attachments, signatures, replacements | Event Detail â†’ Create Report | `POST /corrective-events/{id}/reports`, `PATCH /corrective-reports/{id}` | Engineer/Coordinator/Boss | Auto-save draft every 30s. Queue submit. | Photos, signatures |
| **Report Detail** | View submitted report | View tasks, photos, signatures, print/share | Full report content | Event timeline tap, asset history | `GET /corrective-reports/{id}` | Engineer/Coordinator/Boss | Show cached report | Photos, signatures |
...
| **Schedule Detail** | View PM activity | Execute, view template steps | Template steps, asset, due date, assigned engineers | Schedule list tap | `GET /schedules/{id}` | Engineer | Show cached template | Template diagrams (future) |
| **PM Report Form** | Execute PM report | Complete steps, add photos, sign, submit | Step checklist, notes, photos, signatures | Schedule Detail â†’ Execute | `POST /preventive-reports`, `PATCH /preventive-reports/{id}` | Engineer | Auto-save, queue submit | Photos, signatures |
...
| **Sheet (modal)** | Create event, create report, search, move asset, assign engineers | Slide-up from bottom. Pull-down to dismiss. |
...
| Assign maintenance engineers | `POST /corrective-events/{id}/assign` (future) | Assign Maintenance Engineers | `AssignEngineerViewModel` | None | Invalidate event | Yes |
...
| Assign maintenance engineer | Low | Update UI immediately, revert on failure |
